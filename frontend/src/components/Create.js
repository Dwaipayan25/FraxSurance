...
// const Create = ({ marketplace, nft, truflation, FRAX }) => {
...
//     const approve=await FRAX.approve(nft.address,toWei(price));
//     await approve.wait();
//     // mint nft
...
const Create = ({ marketplace, nft, truflation, FRAX, account }) => {
  ...
  useEffect(() => {
    const getInflationRate = async () => {
      const rate = await truflation.yoyInflation();
      setInflationRate(rate.toString());
      const discount=changeToInt(await marketplace.discountPercent(account));
  ...
  return (
    <div className="container-fluid mt-5">
      <div className="row">
        <h3>Current Inflation Rate: {inflationRate}</h3>
        <h3>Discount you get: {discount ? discount:0}%</h3>
        <main
          role="main"
          className="col-lg-12 mx-auto"
          style={{ maxWidth: "1000px" }}
